...
`ifndef __ahb_sequencer__
`define __ahb_sequencer__
...
class ahb_sequencer extends uvm_sequencer#(ahb_seq_item);
  ...
  uvm_analysis_export#(ahb_seq_item) react_item_export;
  uvm_tlm_analysis_fifo#(ahb_seq_item) react_item_fifo;
  uvm_sequence#(ahb_seq_item) auto_seq;
  ...
  function new(string name = "ahb_sequencer", uvm_component parent = null);
    ...
    uvm_config_db#(ahb_sequencer)::set(parent, "", "sequencer", this); // Put myself in cfg db so others can get access to my reset set/get methods. Look me up by agent + "sequencer".
  endfunction
  ...
  virtual function void build_phase(uvm_phase phase);
    ...
    uvm_config_db#(ahb_policy)::set(this, "*", "policy", plist);
  endfunction
  ...
  virtual task run_auto_seq();
    if (! uvm_config_db#(uvm_sequence#(ahb_seq_item))::get(this, "", "auto_seq", auto_seq)) begin // The auto_seq can be changed during reset.
      if (cfg.role == RESPONDER) begin
        `uvm_fatal("run_auto_seq", "No uvm_sequence#(ahb_seq_item) 'auto_seq' in uvm_config_db. In RESPONDER mode, this sequencer must have a responder sequence to run")
      end
      else begin
