...
`ifndef __gpio_sequencer__
`define __gpio_sequencer__

class gpio_sequencer extends uvm_sequencer#(gpio_seq_item);

  gpio_vip_cfg cfg;
  virtual gpio_interface vif;
  uvm_analysis_export#(gpio_seq_item) react_item_export;
  uvm_tlm_analysis_fifo#(gpio_seq_item) react_item_fifo;
  uvm_sequence#(gpio_seq_item) auto_seq;
  ...
  `uvm_component_utils_begin(gpio_sequencer)
  ...
  function new(string name = "gpio_sequencer", uvm_component parent = null);
    ...
    uvm_config_db#(gpio_sequencer)::set(parent, "", "sequencer", this); // Put myself in cfg db so others can get access to my reset set/get methods. Look me up by agent + "sequencer".
  endfunction
  ...
  //  if (! uvm_config_db#(uvm_sequence#(gpio_seq_item))::get(this, "", "auto_seq", auto_seq)) begin // The auto_seq can be changed during reset.
  //    if (cfg.role == RESPONDER) begin
  //      `uvm_fatal("run_auto_seq", "No uvm_sequence#(gpio_seq_item) 'auto_seq' in uvm_config_db. In RESPONDER mode, this sequencer must have a responder sequence to run")
  //    end
  //    else begin
